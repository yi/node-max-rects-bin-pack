...
(function() {
  ...
  calcOcupiedArea = function(rects, binWidth, binHeight) {
    var bottom, height, k, left, len, rect, right, top, width;
    left = binWidth;
    right = 0;
    top = binHeight;
    bottom = 0;
  ...
  MaxRects = (function() {
    ...
    MaxRects.prototype.calc = function(rects, callback) {
      var err, i, k, len, paddingBothSide, rect, rectIds, uniqueRectIds;
      ...
      for (i = k = 0, len = rects.length; k < len; i = ++k) {
        rect = rects[i];
    ...
    MaxRects.prototype.calcEachRect = function() {
      var bestNode, bestRectIndex, bestScore1, bestScore2, i, k, len, newNode, rect, ref;
      ...
      ref = this.sourceList;
      for (i = k = 0, len = ref.length; k < len; i = k += 1) {
        rect = ref[i];
    ...
    MaxRects.prototype.findPositionForBSSF = function(id, width, height) {
      var bestNode, foundFitNode, k, leftoverHoriz, leftoverVert, len, longSideFit, rect, ref, shortSideFit;
      this.score1 = Number.MAX_VALUE;
      bestNode = new Rectangle(0, 0, 0, 0, id);
      foundFitNode = false;
      ref = this.freeRectangles;
    ...
    MaxRects.prototype.findPositionForBAF = function(id, width, height) {
      var areaFit, bestNode, foundFitNode, k, leftoverHoriz, leftoverVert, len, rect, ref, requestArea, shortSideFit;
      this.score1 = Number.MAX_VALUE;
      bestNode = new Rectangle(0, 0, 0, 0, id);
      foundFitNode = false;
    ...
    MaxRects.prototype.findPositionForBL = function(id, width, height) {
      var bestNode, foundFitNode, k, len, rect, ref, topSideY;
      this.score1 = Number.MAX_VALUE;
      bestNode = new Rectangle(0, 0, 0, 0, id);
      foundFitNode = false;
      ref = this.freeRectangles;
    ...
    MaxRects.prototype.findPositionForCP = function(id, width, height) {
      var bestNode, foundFitNode, k, len, rect, ref, score;
      this.score1 = -1;
      bestNode = new Rectangle(0, 0, 0, 0, id);
      foundFitNode = false;
      ref = this.freeRectangles;
      for (k = 0, len = ref.length; k < len; k += 1) {
        rect = ref[k];
        if (rect.width < width || rect.height < height) {
          continue;
        }
    ...
    MaxRects.prototype.contactPointScoreNode = function(left, top, width, height) {
      var bottom, k, len, rect, ref, right, score;
      score = 0;
      right = left + width;
      bottom = top + height;
